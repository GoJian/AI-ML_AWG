...
<!-- ```{python}
# Train and evaluate SVM on KNN-imputed data
print("SVM on KNN-Imputed Data:")
knn_imputed_data = pd.concat([imp_df_knn2, imp_df_knn2], axis=0)
train_svm_classifier(imp_df_knn2)

# Train and evaluate SVM on Random Sample Imputed data
print("SVM on Random Sample Imputed Data:")
rsi_imputed_data = pd.concat([rsi_df, rsi_df], axis=0)
train_svm_classifier(rsi_df)

# Train and evaluate SVM on MICE-imputed data
print("SVM on MICE-Imputed Data:")
mice_imputed_data = pd.concat([df_imputed, df_imputed], axis=0)
train_svm_classifier(df_imputed) -->
```
...
file_path = '/Users/vaishnavinagesh/Desktop/AI-ML_AWG/fft_image_embeddings.csv'  # Replace with the actual file path
embedding_df = pd.read_csv(file_path)

# Extract mice name, staining technique, and channel into separate columns
embedding_df[['Channel', 'Mice Name', 'Staining Technique']] = embedding_df['filename'].str.extract(r'/[^/]+/[^/]+/[^/]+/([^/]+)/([^_]+)_[^_]+_([^_]+)_')
# Replace multiple substrings using regex
embedding_df['Mice Name'] = (
    embedding_df['Mice Name']
    .str.replace('VG', 'CC2', regex=True)
    .str.replace('V', 'Viv', regex=True)
    .str.replace('FViv', 'CC1', regex=True)
)
# Display the resulting DataFrame
print(embedding_df[['filename', 'Mice Name', 'Staining Technique', 'Channel']])
# Rename all numeric columns in embedding_df to have 'embedding_' prefix
numeric_cols = embedding_df.select_dtypes(include='number').columns
embedding_df.rename(columns={col: f'embedding_{col}' for col in numeric_cols}, inplace=True)
# Save the updated DataFrame to a new file if needed
embedding_df.to_csv('output_file.csv', index=False)
# Rename values in the "Mice Name" column

embedding_df[['filename', 'Mice Name', 'Staining Technique', 'Channel']]
rr9_embedding_all_df = pd.merge(rr9_all_df, embedding_df, left_on="Source Name", right_on="Mice Name", how="inner")

# Rename all numeric columns in embedding_df to have 'embedding_' prefix
numeric_cols = embedding_df.select_dtypes(include='number').columns
embedding_df.rename(columns={col: f'embedding_{col}' for col in numeric_cols}, inplace=True)
# Select only columns that contain "embedding_" in their names
embedding_cols = [col for col in rr9_embedding_all_df.columns if "embedding_" in col]
embedding_data = rr9_embedding_all_df[embedding_cols]

rnaseq_pca_df, scatter_plt  = perform_pca(rr9_embedding_all_df,dataset_name=embedding_data)
scatter_plt.show()
```

## Isolating only Phenotypic Genes for Imputation and further training
```{python}
flight_relevant_genes = rr9_embedding_all_df[rr9_embedding_all_df['Group'] == 'F'][found_genes+genes_predictive_of_phenotypes + ['Source Name', 'Group']]

non_flight_relevant_genes = rr9_embedding_all_df[rr9_embedding_all_df['Group'] != 'F'][found_genes+genes_predictive_of_phenotypes + ['Source Name', 'Group']]

# Prepare a dictionary of all relevant dataframes (excluding sc_gp, which is just group info)
modalities = {
    'tunel': tunel,
    'hne': hne,
    'zo1': zo1,
...
# For each modality, get the columns for flight and non-flight data
flight_data_dict = {}
non_flight_data_dict = {}

for name, df in modalities.items():
    cols = df.columns.to_list() + ['Source Name', 'Group']
    flight_data_dict[name] = rr9_embedding_all_df[rr9_embedding_all_df['Group'] == 'F'][cols]
    non_flight_data_dict[name] = rr9_embedding_all_df[rr9_embedding_all_df['Group'] != 'F'][cols]

# Example: flight_data_dict['tunel'] gives flight samples for tunel data
# To concatenate all modalities for flight and non-flight:
flight_all_modalities = pd.concat(list(flight_data_dict.values()), axis=1)
non_flight_all_modalities = pd.concat(list(non_flight_data_dict.values()), axis=1)

# Remove duplicate columns (from overlapping 'Source Name' and 'Group')
flight_all_modalities = flight_all_modalities.loc[:,~flight_all_modalities.columns.duplicated()]
non_flight_all_modalities = non_flight_all_modalities.loc[:,~non_flight_all_modalities.columns.duplicated()]


merged_flight_data = pd.merge(
    flight_relevant_genes,
    flight_all_modalities,
    on=['Source Name', 'Group'],
    how='outer'
)
...
## Bagging Regressor for Final Imputation from Embeddings Flight Data
```{python}
bagger = BaggingRegressor(random_state=2)
itera_bagger = IterativeImputer(random_state=2, initial_strategy='median', estimator=bagger, max_iter=50, verbose=2, tol=0.01)
# Before fitting, get columns that are NOT all NaN
cols_to_impute = merged_flight_data.drop(columns=['Source Name', 'Group']).columns[
    merged_flight_data.drop(columns=['Source Name', 'Group']).notna().any()
].tolist()

# Fit and transform only these columns
itera_bagger.fit(merged_flight_data[cols_to_impute])
df_bag_imputed_flight = itera_bagger.transform(merged_flight_data[cols_to_impute])
df_bag_imputed_flight = pd.DataFrame(df_bag_imputed_flight, columns=cols_to_impute)
df_bag_imputed_flight['Source Name'] = merged_flight_data['Source Name']
df_bag_imputed_flight['Group'] = merged_flight_data['Group']
...
# Fit and transform only these columns
itera_bagger.fit(merged_non_flight_data[cols_to_impute])
df_bag_imputed_non_flight = itera_bagger.transform(merged_non_flight_data[cols_to_impute])
df_bag_imputed_non_flight = pd.DataFrame(df_bag_imputed_non_flight, columns=cols_to_impute)
df_bag_imputed_non_flight['Source Name'] = merged_non_flight_data['Source Name']
df_bag_imputed_non_flight['Group'] = merged_non_flight_data['Group']
corr_matrix = df_bag_imputed_non_flight.drop(columns=['Source Name','Group']).corr()
sns.heatmap(corr_matrix, cmap='coolwarm')
...
```{python}
combined_df = pd.concat([df_bag_imputed_non_flight, df_bag_imputed_flight], ignore_index=True)
